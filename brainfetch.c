...
int printLinePosition(char* code, int pos) {
	...
	int ch = 1;
	for (int i = 0; i < pos; i++) {
		if (code[i] == 10) {	// LF (newline) character
			li++;
			ch = 1;
		}
		ch++;
...
int main(int argc, char** argv) {
	...
	if (argc < 2) {				// end program if no file specified/found
		printf("Usage: %s [code].bf\n", argv[0]);
		return 1;
	}
	...
	int fileSize = loadCode(f, prg, debugMode);

	if (!fileSize || !testCode(prg, fileSize)) {	// run tests, exit program if failed
		return 0;
	}
	...
	while (head < fileSize) {	// Main interpretation loop. Each character/command is interpreted one at a time.
		switch (prg[head]) {
			...
			case ',':
				...
				break;
			case '[':
				if (!mem[ptr]) {				// check for nonzero value
